...
impl AgentBuilder {
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        let skip_hosted_web_search = match &self.web_search_config {
            xai_grok_tools::implementations::web_search::WebSearchConfig::Enabled {
                base_url,
                model,
                ..
            } => xai_grok_tools::implementations::web_search::is_searxng_endpoint(base_url, model),
            xai_grok_tools::implementations::web_search::WebSearchConfig::Disabled => false,
        };
        ...
        if use_backend_search {
            if web_search_enabled
                && !skip_hosted_web_search
                && definition.hosted_tool_allowed("web_search")
            {
...
mod tests {
    ...
    #[tokio::test]
    async fn searxng_web_search_does_not_register_xai_hosted_web_search() {
        use xai_grok_tools::computer::local::LocalTerminalBackend;
        use xai_grok_tools::implementations::web_search::WebSearchConfig;
        use xai_grok_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_grok_build();
        ...
        .from_definition(def)
        .with_web_search_config(WebSearchConfig::local_searxng(None, None))
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, xai_grok_sampling_types::HostedTool::WebSearch { .. })),
            "SearxNG-backed search must not advertise xAI hosted WebSearch, got: {hosted:?}"
        );
        let has_web_search_fn = agent
            .tool_definitions()
            .await
        ...
        assert!(
            has_web_search_fn,
            "function web_search tool must remain for SearxNG"
        );
    }
