...
//! grok-home (`$GROK_HOME` or `<home>/.grok-local`). Shared by `xai-grok-config`
...
/// User-home directory name. Distinct from the official CLI's `.grok` so the
/// two installs do not share config, auth, sockets, or session state.
pub const DEFAULT_GROK_DIRNAME: &str = ".grok-local";

/// `<home>/.grok-local`, canonicalized via `dunce` (not `std::fs::canonicalize`,
...
/// `$GROK_HOME` verbatim when non-empty, else `<home>/.grok-local`. The env value is
...
/// The default `<home>/.grok-local`, used when `$GROK_HOME` is unset.
...
mod tests {
    ...
    fn default_grok_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(DEFAULT_GROK_DIRNAME));
    }
    ...
    #[test]
    fn default_home_does_not_collide_with_official_grok_dir() {
        assert_eq!(DEFAULT_GROK_DIRNAME, ".grok-local");
        assert_ne!(DEFAULT_GROK_DIRNAME, ".grok");
    }
}
