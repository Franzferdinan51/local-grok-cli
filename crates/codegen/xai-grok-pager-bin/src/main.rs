            ...
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        "grokLocal": env!("LOCAL_VERSION_WITH_COMMIT"),
                        "grokBuild": env!("VERSION_WITH_COMMIT"),
                        "sourceRev": xai_grok_version::source_rev(),
                        "currentVersion": env!("LOCAL_VERSION_WITH_COMMIT"),
...
mod tests {
    ...
    fn version_output_writer_preserves_channel_aware_contract() {
        ...
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("grok-local "));
            assert!(output.contains("grok-build "));
            assert!(output.contains(env!("LOCAL_VERSION_WITH_COMMIT")));
