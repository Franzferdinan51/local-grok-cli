...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("grok-local{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        .args(["build", "-p", "xai-grok-pager-bin", "--bin", "grok-local"])
        .stdin(Stdio::null())
...
/// 2. `CARGO_BIN_EXE_grok_local` / `CARGO_BIN_EXE_xai-grok-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-grok-pager-bin` (the composition-
///    root package that owns the `grok-local` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    for key in ["CARGO_BIN_EXE_grok_local", "CARGO_BIN_EXE_xai-grok-pager"] {
