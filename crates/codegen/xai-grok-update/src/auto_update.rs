...
/// Local-first: unset and `false` must not contact an updater. Only `Some(true)` may.
pub(crate) fn auto_update_allows_network(auto_update: Option<bool>) -> bool {
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    let current_config = config::load_config().await;
    if !auto_update_allows_network(current_config.cli.auto_update) {
...
pub async fn run_update_if_available(
    ...
    let current_config = config::load_config().await;
    let auto_update = auto_update_allows_network(current_config.cli.auto_update);
    if !auto_update {
