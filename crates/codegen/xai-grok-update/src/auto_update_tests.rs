...
#[test]
fn auto_update_allows_network_is_fail_closed_when_unset() {
    assert!(
        !auto_update_allows_network(None),
        "first-run (unset) must not contact an updater"
    );
    assert!(
        !auto_update_allows_network(Some(false)),
        "explicit off must not contact an updater"
    );
    assert!(
        auto_update_allows_network(Some(true)),
        "explicit on may contact an updater"
    );
}
