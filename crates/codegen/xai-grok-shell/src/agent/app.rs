...
mod tests {
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        // GROK_LOCAL: the fork fails remote fetch CLOSED by default, which
        // leaves no policy channel; this test exercises the fetch-on path, so
        // it opts in through a throwaway `$GROK_HOME/config.toml`.
