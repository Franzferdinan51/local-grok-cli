...
/// Default base URL for the public xAI API (kept for explicit overrides).
pub const XAI_API_BASE_URL_DEFAULT: &str = "https://api.x.ai/v1";
/// Local-first inference: LM Studio's OpenAI-compatible server.
...
impl EndpointsConfig {
    pub fn has_custom_endpoint(&self) -> bool {
        blank_as_unset(&self.models_base_url).is_some()
            || blank_as_unset(&self.models_list_url).is_some()
    }
    ...
    pub(crate) fn resolve_inference_base_url(&self) -> String {
        if let Some(url) = blank_as_unset(&self.models_base_url) {
            return url;
        }
        if blank_as_unset(&self.cli_chat_proxy_base_url).is_some() {
    ...
    pub(crate) fn resolve_models_list_url(&self) -> String {
        if let Some(url) = blank_as_unset(&self.models_list_url) {
            return url;
        }
        format!(
            "{}/models",
            self.resolve_inference_base_url().trim_end_matches('/')
        )
    }
}

/// Normalize an OpenAI-compatible base (`…` or `…/v1`) to `…/v1`.
pub fn normalize_openai_compat_base_url(raw: &str) -> String {
    let trimmed = raw.trim().trim_end_matches('/');
    if trimmed.is_empty() {
        return LM_STUDIO_BASE_URL_DEFAULT.to_owned();
    }
...
/// True for loopback inference hosts (LM Studio, Ollama, etc.).
...
/// LM Studio / local OpenAI-compat URL: `GROK_MODELS_BASE_URL` > `LM_STUDIO_URL` > localhost:1234.
pub fn default_local_inference_url() -> String {
    env_string("GROK_MODELS_BASE_URL")
        .or_else(|| env_string("LM_STUDIO_URL"))
        .map(|s| normalize_openai_compat_base_url(&s))
        .unwrap_or_else(|| LM_STUDIO_BASE_URL_DEFAULT.to_owned())
}
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("GROK_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: env_string("GROK_XAI_API_BASE_URL")
                .map(|s| normalize_openai_compat_base_url(&s))
                .unwrap_or_else(default_local_inference_url),
            alpha_test_key: None,
...
pub(crate) fn resolve_model_list(
    ...
    let local = is_local_inference_url(&inference_url);
    if local {
        tracing::info!(
            inference_url = %inference_url,
            "local inference: catalog is LM Studio's live model list only"
        );
    } else if cfg.endpoints.has_custom_endpoint() {
        tracing::info!(
...
fn default_models(endpoints: &EndpointsConfig) -> IndexMap<String, ModelEntryConfig> {
    ...
    entries
        ...
        .map(|m| {
            ...
            let local_slug = crate::models::default_model();
            let model = if m.model == "local-model" {
            ...
            let base_url = endpoints.resolve_inference_base_url();
            let local = is_local_inference_url(&base_url);
            let config = ModelEntryConfig {
                ...
                api_key: local.then(|| LM_STUDIO_DUMMY_API_KEY.to_owned()),
...
/// Manual `Debug`: entries are logged on catalog refresh, so the key value (and
/// the key field name) must never appear in debug output. Presence is still visible.
...
pub(crate) fn resolve_credentials(
    ...
    } else if is_local_inference_url(&info.base_url) {
        (
            Some(LM_STUDIO_DUMMY_API_KEY.to_owned()),
            info.base_url.clone(),
            xai_chat_state::AuthType::ApiKey,
        )
    } else {
