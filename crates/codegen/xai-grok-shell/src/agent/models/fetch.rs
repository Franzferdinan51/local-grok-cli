...
fn fetch_models_uncommitted(
    ...
    if crate::agent::config::is_local_inference_url(&endpoints.resolve_inference_base_url()) {
        return match super::lm_studio::fetch_local_inference_catalog(endpoints) {
            Some(models) => ModelsPrefetch::Cached(models),
...
pub(crate) fn resolve_prefetch_env_from_parts(
    ...
    let local =
        crate::agent::config::is_local_inference_url(&endpoints.resolve_inference_base_url());
    if !remote_fetch_enabled && !local {
        tracing::info!("startup model/settings prefetch skipped: remote_fetch disabled");
        return None;
    }

    let model_fetch_auth = if local {
        ModelFetchAuth::CustomEndpoint
    } else {
        ModelFetchAuth::resolve(&endpoints, auth.is_some())
    };
