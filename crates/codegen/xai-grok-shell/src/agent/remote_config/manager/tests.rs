...
/// Upstream-shaped endpoints for tests that exercise the xAI fetch path.
/// The fork's `EndpointsConfig::default()` points at local LM Studio, which
/// takes the local-inference short-circuit instead of the mock endpoint.
fn remote_endpoints() -> config::EndpointsConfig {
    let mut endpoints = config::EndpointsConfig::default();
    endpoints.cli_chat_proxy_base_url = Some("https://cli-chat-proxy.grok.com/v1".to_owned());
    endpoints.models_base_url = None;
    endpoints.models_list_url = None;
    endpoints.deployment_key = None;
    endpoints
}

fn remote_config() -> config::Config {
    let mut cfg = config::Config::default();
    cfg.endpoints = remote_endpoints();
    cfg
}
...
async fn catalog_retry_recovers_after_endpoint_returns() {
    ...
    // Fork path: remote_fetch is off by default, so the retry loop takes the
    // local-inference fetch. Serve a mock LM Studio catalog that fails once,
    // then succeeds, and assert the retry recovers from it. The xAI endpoint
    // mock stays installed to prove the local path never consults it.
    ...
    std::thread::spawn(move || {
        for stream in listener.incoming().take(6) {
            ...
            let attempt = hits_server.fetch_add(1, Ordering::SeqCst);
    ...
    cfg.endpoints.models_base_url = Some(format!("http://127.0.0.1:{port}/v1"));
    ...
    assert!(
        recovered,
        "catalog retry did not recover after the local server returned"
    );
    assert!(mgr.models().contains_key("retry-model"));
    assert!(
...
/// Fork adaptation of upstream `auth_refresh_watcher_refetches_on_notify`.
/// Remote cfg isolates the test from the local-inference short-circuit, so the
/// only variable is the process-global `remote_fetch` gate (default false on
/// the fork, so the watcher must skip; true keeps upstream refetch behavior).
#[tokio::test]
async fn auth_refresh_watcher_honors_remote_fetch_gate() {
    use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    if crate::util::config::resolve_remote_fetch_enabled() {
        ...
        assert!(
            ...
            "watcher must not fetch when remote_fetch is disabled (fork default)"
        );
        assert_eq!(
            calls.load(Ordering::SeqCst),
            0,
            "disabled watcher must not hit the models endpoint"
        );
    }
}
...
async fn hanging_fetch_does_not_block_refresh() {
    let mgr = cold_manager(remote_config(), Arc::new(HangingEndpoint));
...
async fn spawn_background_refresh_never_blocks_on_a_hanging_endpoint() {
    ...
    // Fork path: remote_fetch is off by default, so the refresh runs against
    // the local LM Studio server. A slow server must not block the spawn call:
    // the fetch happens on a spawned blocking worker while the caller proceeds.
    let listener = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
    let port = listener.local_addr().unwrap().port();
    let hits = Arc::new(AtomicUsize::new(0));
    let hits_server = hits.clone();
    ...
    assert!(
        ...
        "spawn blocked on the hanging fetch; it must dispatch to the background",
    );
    assert!(
        !mgr.has_fetched_real_catalog(),
    ...
    assert!(recovered, "background refresh never completed");
    assert!(mgr.models().contains_key("bg-model"));
...
fn from_config_without_prefetch_produces_usable_catalog() {
    let tmp = tempfile::TempDir::new().unwrap();
    let auth_manager = Arc::new(AuthManager::new(tmp.path(), GrokComConfig::default()));
    // Remote cfg: the fork's local default serves LM Studio's live list only,
    // so a cold boot with no server is an empty catalog by design.
...
fn models_commit_gate_detects_account_switch() {
    ...
    // The commit gate consults process-global remote_fetch (default off on the
    // fork) and policy repair. Only assert the identity matrix when no gate
    // trips; otherwise the fork-relevant property is that nothing is cached.
    if crate::util::config::resolve_remote_fetch_enabled()
        && !crate::managed_config::policy_repair_pending()
    {
        ...
        for (name, live) in [
            ("same-account", scope_for("account-a")),
            ("switched-account", scope_for("account-b")),
            ("moved-origin", moved),
        ] {
            assert!(
                matches!(
                    evaluate_models_commit(&expected, &live),
                    Commit::Abandon | Commit::Retry
                ),
                "{name}: without remote_fetch no commit may cache or serve",
            );
        }
    }
}
...
fn resolve_live_keeps_fetch_origin_when_disk_auth_absent() {
    ...
    // The commit consults process-global remote_fetch (default off on the fork)
    // and policy repair; without them it must abandon, never cache.
    if crate::util::config::resolve_remote_fetch_enabled()
        && !crate::managed_config::policy_repair_pending()
    {
        assert!(
        ...
        assert!(
            ...
            "without remote_fetch a disk-absent commit must not cache",
        );
    }
}
...
fn resolve_deployment_key_when_no_session_or_api_key() {
    ...
    let endpoints = config::EndpointsConfig {
        ...
        cli_chat_proxy_base_url: Some("https://cli-chat-proxy.grok.com/v1".to_owned()),
...
fn prefetch_env_resolves_when_remote_fetch_enabled() {
    ...
    let mut remote_only = config::EndpointsConfig::default();
    remote_only.cli_chat_proxy_base_url = Some("https://cli-chat-proxy.grok.com/v1".to_owned());
    remote_only.models_base_url = None;
    assert!(
        resolve_prefetch_inputs_from_parts(None, remote_only, true).is_none(),
        "no credentials and no custom/local endpoint must stay a no-prefetch launch",
    );
    assert!(
        resolve_prefetch_inputs_from_parts(None, config::EndpointsConfig::default(), false)
            .is_some(),
        "LM Studio local inference still prefetches the live model list",
    );
}
