...
impl ModelsManager {
    ...
    pub(crate) fn ensure_local_inference_model(&self, id: &str) {
        ...
        let entry = super::lm_studio::entry_for_id(id, &base);
        let mut cat = self.inner.catalog.write();
        if !cat.models.contains_key(id) {
    ...
    /// Local-first: local LM Studio inference waits even when `remote_fetch_enabled` is false.
    pub(crate) async fn wait_for_first_catalog(&self, remote_fetch_enabled: bool) -> bool {
        let remote_fetch_enabled = remote_fetch_enabled
            || config::is_local_inference_url(
    ...
    pub(crate) async fn on_auth_changed(&self) {
        ...
        *self.inner.fetch_auth.write() = fetch_auth;
        let local = config::is_local_inference_url(&config.endpoints.resolve_inference_base_url());
        // No session but the endpoint needs one: a fetch would 401, so skip it and reset to this identity's bundled catalog
        if !local && !has_session && fetch_auth == ModelFetchAuth::Session {
            self.clear();
        ...
        let remote_fetch_enabled = crate::util::config::resolve_remote_fetch_enabled() || local;
    ...
    pub fn spawn_background_refresh(&self) {
        ...
        self.spawn_background_refresh_inner(
    ...
    async fn fetch_and_apply_inner(&self, remote_fetch_enabled: bool) {
        let cfg = self.inner.cfg.read().clone();
        let local = config::is_local_inference_url(&cfg.endpoints.resolve_inference_base_url());
        if !remote_fetch_enabled && !local {
            tracing::info!("model catalog refresh skipped: remote_fetch disabled");
            return;
        }
        let attempt = FetchAttemptGuard::begin(&self.inner);
        ...
        let endpoint = self.inner.endpoint.clone();
        let new_prefetched = if local {
            let endpoints = cfg.endpoints.clone();
            tokio::task::spawn_blocking(move || {
                super::lm_studio::fetch_local_inference_catalog(&endpoints)
            })
            .await
            .ok()
            .flatten()
