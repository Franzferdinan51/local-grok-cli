...
#[test]
fn default_inference_url_is_lm_studio() {
    assert_eq!(LM_STUDIO_BASE_URL_DEFAULT, "http://127.0.0.1:1234/v1");
    assert_eq!(
        normalize_openai_compat_base_url("http://127.0.0.1:1234"),
        "http://127.0.0.1:1234/v1"
    );
    assert_eq!(
        normalize_openai_compat_base_url("http://127.0.0.1:1234/v1/"),
        "http://127.0.0.1:1234/v1"
    );
    assert!(is_local_inference_url(LM_STUDIO_BASE_URL_DEFAULT));
    assert!(!is_local_inference_url("https://api.x.ai/v1"));
}

#[test]
#[serial]
fn endpoints_default_points_at_lm_studio() {
    let _g1 = EnvGuard::unset("GROK_MODELS_BASE_URL");
    let _g2 = EnvGuard::unset("LM_STUDIO_URL");
    let _g3 = EnvGuard::unset("GROK_XAI_API_BASE_URL");
    let ep = EndpointsConfig::default();
    assert_eq!(ep.resolve_inference_base_url(), LM_STUDIO_BASE_URL_DEFAULT);
    assert_eq!(
        ep.resolve_models_list_url(),
        "http://127.0.0.1:1234/v1/models"
    );
    assert!(
        !ep.has_custom_endpoint(),
        "unset models_base_url is not a custom endpoint; LM Studio is the inference fallback"
    );
}

#[test]
fn resolve_credentials_uses_lm_studio_dummy_key_on_localhost() {
    let model = test_model_entry("local-model", LM_STUDIO_BASE_URL_DEFAULT, None, None, None);
    let creds = resolve_credentials(&model, None);
    assert_eq!(creds.base_url, LM_STUDIO_BASE_URL_DEFAULT);
    assert_eq!(creds.api_key.as_deref(), Some(LM_STUDIO_DUMMY_API_KEY));
}

#[test]
#[serial]
fn default_model_catalog_is_chat_completions_on_lm_studio() {
    let _g1 = EnvGuard::unset("GROK_MODELS_BASE_URL");
    let _g2 = EnvGuard::unset("LM_STUDIO_URL");
    let _g3 = EnvGuard::unset("GROK_XAI_API_BASE_URL");
    let cfg = Config::default();
    let live = crate::agent::remote_config::catalog_from_lm_studio_json(
        &serde_json::json!({
            "data": [
                {"id": "ornith-1.5-35b-a3b", "object": "model"},
                {"id": "bonus-local", "object": "model"}
            ]
        }),
        LM_STUDIO_BASE_URL_DEFAULT,
    );
    let models = resolve_model_list(&cfg, Some(live));
    assert_eq!(models.len(), 2);
    assert!(models.contains_key("ornith-1.5-35b-a3b"));
    assert!(models.contains_key("bonus-local"));
    assert!(!models.contains_key("local-model"));
    assert!(!models.contains_key("grok-4.6"));
    let entry = models.get("ornith-1.5-35b-a3b").unwrap();
    assert_eq!(entry.info.base_url, LM_STUDIO_BASE_URL_DEFAULT);
...
fn hidden_default_web_search_resolution_is_explicit_and_responses_only() {
    ...
    // GROK_LOCAL: the fork's hidden web-search default is the local model on
    // the local server, not the xAI proxy (still resolved as Responses-only).
    assert_eq!(resolved.base_url, LM_STUDIO_BASE_URL_DEFAULT);
...
fn session_resolver_is_not_stamped_onto_third_party_samplers() {
    ...
    let mut local_sampler = SamplerConfig {
        base_url: EndpointsConfig::default().resolve_inference_base_url(),
        ..SamplerConfig::default()
    };
    stamp_session_local_sampler_fields(&mut local_sampler, &session_cfg, None, None);
    // GROK_LOCAL: the fork classifies the local inference URL as third-party,
    // so session JWTs are never stamped onto requests to the user's own server.
    assert!(
        local_sampler.bearer_resolver.is_none(),
        "local-inference aux samplers must not receive the session refresh behavior"
    );
    assert_eq!(
        local_sampler
            .conversation_group_id
...
fn default_models_dual_endpoint_routing() {
    ...
    for (model_id, entry) in default_model_entries(&endpoints) {
        ...
        // GROK_LOCAL: the fork's default entry is the local LM Studio model,
        // which stays on the local server even with a session token.
        assert_eq!(
            session_creds.base_url, LM_STUDIO_BASE_URL_DEFAULT,
            "{model_id}: SessionToken must route to the local inference server on the fork"
        );
...
#[test]
fn has_own_credentials_local_default_carries_lm_studio_dummy_key() {
    let endpoints = EndpointsConfig::default();
    for (model_id, entry) in default_model_entries(&endpoints) {
        // GROK_LOCAL: the fork's local default carries the LM Studio dummy
        // credential so keyless local servers accept requests.
        assert!(
            entry.has_own_credentials(),
            "{model_id}: fork default model must carry the LM Studio dummy credential"
        );
        assert_eq!(
            entry.api_key.as_deref(),
            Some(LM_STUDIO_DUMMY_API_KEY),
            "{model_id}: fork default credential must be exactly the dummy key"
        );
    }
...
#[test]
fn e2e_default_model_with_session_stays_on_local_inference() {
    // GROK_LOCAL: blank config + local inference means an empty built-in
    // catalog (the live list comes from LM Studio at runtime), so the default
    // entry is declared explicitly; session auth keeps it on the local server.
    let dm = crate::models::default_model();
    let (_, models) = resolve_models_from_toml(
        &format!(
            r#"
    ...
    assert_eq!(
        sampling.base_url, LM_STUDIO_BASE_URL_DEFAULT,
        "session auth should keep the local default model on the local server, not cli-chat-proxy"
    );
}
#[test]
#[serial]
fn e2e_default_model_with_external_api_key_stays_on_local_inference() {
    // GROK_LOCAL: blank config + local inference means an empty built-in
    // catalog (the live list comes from LM Studio at runtime), so the default
    // entry is declared explicitly; the external key is ambient auth only and
    // routing stays local-first.
    let dm = crate::models::default_model();
    let (_, models) = resolve_models_from_toml(
        &format!(
            r#"
    ...
    assert_eq!(
        sampling.base_url, LM_STUDIO_BASE_URL_DEFAULT,
        "external API key must not reroute the local default model to api.x.ai"
    );
    unsafe { std::env::remove_var("XAI_API_KEY") };
}
#[test]
...
fn e2e_duplicate_model_field_both_entries_survive() {
    let dm = crate::models::default_model();
    // GROK_LOCAL: blank config + local inference means an empty built-in
    // catalog, so the default entry is declared explicitly alongside the user
    // entry that shares its `model` field.
    let (_, models) = resolve_models_from_toml(
        &format!(
            r#"
            [model."{dm}"]
    ...
    // GROK_LOCAL: session auth keeps the local default on the local server.
    assert_eq!(sampling.base_url, LM_STUDIO_BASE_URL_DEFAULT);
}
#[test]
...
#[test]
fn e2e_default_endpoint_does_not_inject_xai_placeholders() {
    let cfg = Config::default();
    let resolved = resolve_model_list(&cfg, None);
    assert!(
        !resolved.contains_key("grok-4.6"),
        "xAI bundled models must not appear in the local catalog"
    );
    assert!(
        resolved.is_empty(),
        "without a live LM Studio list the picker stays empty, got {resolved:?}"
    );
}
#[test]
...
fn aux_endpoints_resolve_to_proxy_never_inference() {
    ...
    // GROK_LOCAL: the fork routes inference to the local LM Studio server by
    // default; only the auxiliary endpoints follow the proxy.
    assert_eq!(cfg.resolve_inference_base_url(), LM_STUDIO_BASE_URL_DEFAULT);
    // GROK_LOCAL: the models list comes from the local server (LM Studio's
    // live list), not the proxy.
    assert_eq!(
        cfg.resolve_models_list_url(),
        format!("{LM_STUDIO_BASE_URL_DEFAULT}/models")
    );
    assert_eq!(
...
fn global_extra_headers_apply_to_model_without_override() {
    let dm = crate::models::default_model();
    // GROK_LOCAL: blank config + local inference means an empty built-in
    // catalog, so the default entry is declared explicitly (with no
    // per-model headers, so the global ones must apply).
    let (_, models) = resolve_models_from_toml(
        &format!(
            r#"
            [models]
...
fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
    ...
    assert!(
        !no_p.contains_key("grok-4.6"),
        "local catalog must not fall back to bundled xAI models"
    );
}
#[test]
