...
/// GROK_LOCAL: the fork pins API-key auth on a local host, so the fallthrough
/// is `xai.api_key` and never advertises interactive `grok.com` login.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_api_key_without_credentials() {
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(XAI_API_KEY_METHOD_ID),
        "fork: no API-key creds and no kill switch -> xai.api_key, never interactive grok.com",
    );
}
...
async fn auto_gc_declines_until_the_remote_answer_settles() {
    let (_home, _env) = search_index_env();
    // GROK_LOCAL: the fork fails remote fetch CLOSED by default (local-first:
    // no egress without an explicit pin); this test exercises the fetch-on
    // path, so it opts in through the throwaway `$GROK_HOME/config.toml`.
    std::fs::write(
...
async fn access_gate_does_not_leak_verdict_across_identities() {
    ...
    // GROK_LOCAL: the fork fails remote fetch CLOSED by default (local-first:
    // no egress without an explicit pin); the provisional-open arm under test
    // needs the fetch-on path, so opt in via a throwaway `$GROK_HOME`.
    let fetch_home = tempfile::tempdir().unwrap();
    std::fs::write(
        fetch_home.path().join("config.toml"),
        "[features]\nremote_fetch = true\n",
    )
    .unwrap();
    let _fetch_env = xai_grok_test_support::EnvGuard::set("GROK_HOME", fetch_home.path());
    let auth_a = GrokAuth {
...
async fn post_auth_settings_xai_upgrades_writeback_emits_and_opens_gate() {
    ...
    // GROK_LOCAL: the fork fails remote fetch CLOSED by default (local-first:
    // no egress without an explicit pin); this test exercises the fetch-on
    // path, so it opts in through a throwaway `$GROK_HOME/config.toml`.
    let fetch_home = tempfile::tempdir().unwrap();
    std::fs::write(
        fetch_home.path().join("config.toml"),
        "[features]\nremote_fetch = true\n",
