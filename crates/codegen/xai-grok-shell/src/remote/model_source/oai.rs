        ...
        match self.endpoint.auth {
            EndpointAuth::ApiKey => {
                let api_key = crate::agent::auth_method::read_xai_api_key_env()
                    ...
                    .unwrap_or_else(|_| {
                        if crate::agent::config::is_local_inference_url(&self.inference_base_url)
                            || crate::agent::config::is_local_inference_url(&self.endpoint.url)
                        {
...
impl ListModelsEndpoint {
    fn from_endpoints(endpoints: &EndpointsConfig, fetch_auth: ModelFetchAuth) -> Self {
        if endpoints.has_custom_endpoint()
            ...
            || fetch_auth == ModelFetchAuth::CustomEndpoint
        {
            Self {
...
mod tests {
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(
            default.resolve_inference_base_url(),
        ...
        let mut no_custom = EndpointsConfig::default();
