...
/// Canonical grok application path: `$GROK_HOME/bin/grok-local` (Unix) or `grok-local.exe` (Windows).
...
/// System-wide config directory: `/etc/grok-local/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/grok-local"))
    } else {
        None
    }
}
...
mod tests {
    ...
    #[test]
    fn application_binary_is_grok_local() {
        let tmp = TempDir::new().unwrap();
        let path = grok_application_in(tmp.path());
        let name = path.file_name().and_then(|n| n.to_str()).unwrap();
        assert!(
            name.starts_with("grok-local"),
            "expected grok-local binary, got {name}"
        );
        assert!(!name.eq("grok") && !name.eq("grok.exe"));
    }
}
