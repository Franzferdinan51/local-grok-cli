...
impl WebSearchConfig {
    /// Local SearxNG backend. No API key; default URL is
    /// [`super::default_searxng_url`].
    pub fn local_searxng(
        allowed_domains: Option<Vec<String>>,
        excluded_domains: Option<Vec<String>>,
    ) -> Self {
        Self::Enabled {
            api_key: String::new(),
            base_url: super::default_searxng_url(),
...
mod tests {
    ...
    #[test]
    fn local_searxng_is_enabled_without_api_key() {
        let config = WebSearchConfig::local_searxng(None, None);
        match config {
            WebSearchConfig::Enabled {
                ...
                assert_eq!(model, "searxng");
                assert!(
                    crate::implementations::web_search::is_searxng_endpoint(&base_url, &model),
                    "local_searxng must resolve to a SearxNG endpoint, got {base_url}"
                );
            }
            WebSearchConfig::Disabled => panic!("local_searxng must be Enabled"),
        }
    }
