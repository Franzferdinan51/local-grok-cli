...
impl WebSearchClient {
    ...
    pub async fn search(
        ...
        if super::is_searxng_endpoint(&self.base_url, &self.model) {
            let (content, pairs) = self.search_searxng(query, allowed_domains).await?;
            let citations = pairs.into_iter().map(|(_t, url)| url).collect();
    ...
    pub async fn search_with_titles(
        ...
        if super::is_searxng_endpoint(&self.base_url, &self.model) {
            return self.search_searxng(query, allowed_domains).await;
        }
        let (allowed, excluded) = self.resolve_filters(allowed_domains);
        let request = self.build_request_json(query, allowed, excluded)?;
        let url = format!("{}/responses", self.base_url.trim_end_matches('/'));
    ...
    /// Query a local SearxNG instance (`GET /search?q=&format=json`).
    async fn search_searxng(
        ...
        let (allowed, excluded) = self.resolve_filters(allowed_domains);
        let url = super::searxng::search_url(&self.base_url);
        let response = self
            .http
            .get(&url)
        ...
        let pairs: Vec<(String, String)> = hits
            ...
            .collect();
        let content = super::searxng::format_searxng_results(query, &hits);
        Ok((content, pairs))
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn searxng_search_uses_json_endpoint_not_xai_responses() {
        ...
        let (content, citations) = client
            ...
            .await
            .expect("searxng search must succeed");
        assert!(
